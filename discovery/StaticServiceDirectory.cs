...
public sealed class StaticServiceDirectory : IServiceDirectory
{
    ...
    public StaticServiceDirectory(IOptions<DiscoveryOptions> options, ILogger<StaticServiceDirectory> logger)
    {
        _logger = logger;
    ...
    public ValueTask<ServiceLookup> ResolveAsync(string serviceName, int? port, CancellationToken cancellationToken)
    {
        ...
        _logger.LogWarning("No Discovery:Fallback entry for {Service}; resolving it to nothing", serviceName);
